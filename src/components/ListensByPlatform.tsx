...
const ListensByPlatform: React.FC<ListensByPlatformProps> = ({ history, data }) => {
    ...
    useLayoutEffect(() => {
        ...
        if (data && data.length > 0) {
            ...
            chartData = (history || []).reduce<{ platform: string; value: number }[]>((result, item) => {
                const platform = (item.platform || "Unknown").trim();

                const index = result.findIndex((entry) => entry.platform === platform);
                if (index >= 0) {
                    result[index].value += 1;
                } else {
                    result.push({platform, value: 1});
                }
